...
def transaction_db(example_data, test_db='test_tracker.db'):
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS transactions (id INT, amount FLOAT, category TEXT, date DATE, description TEXT)")
    for tup in example_data: 
...
def test_selectAll(transaction_db, example_data_dict):
    '''
...
def test_add(transaction_db, example_data_dict):
    ...
    updated_example_data_dict.append({'id': tup[0], 'amount': tup[1], 'category': tup[2], 'date': tup[3], 'description': tup[4]})
